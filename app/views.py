...
from .models import Product, Pr_class, Sub_Pr_class, Account, Cart,  Article
...
def index(request):
	...
	request_list = str(my_request).replace('_', ' ').split()
	if need_replace:
	...
	if request_list!=['None'] and request_list[1]!='none' and str(request_list[1])!='None':
		...
		if class_str[0]=='^':
			...
			if orderBy:
				...
				objects = Article.objects.all().filter(product__sub_product_class=sub_class_obj_list)
